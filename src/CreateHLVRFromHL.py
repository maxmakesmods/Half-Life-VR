...
# Must be run from the 'src' directory within a freshly (and completely) cloned Half-Life: VR repository (https://github.com/maxmakesmods/Half-Life-VR).
...
def main(argv):
    ...
    if not os.path.basename(script_location) == "src" or not os.path.exists(os.path.join(script_location, 'HLVR.sln')):
        print('Error: Unexpected script location. Script must be run from the "src" folder in a fully checked out Half-Life: VR repository (https://github.com/maxmakesmods/Half-Life-VR).')
        sys.exit(2)
    ...
    for dll in dlls_to_copy:
        if not os.path.exists(os.path.join(script_location, dll)):
            print('Error: Library "'+dll+'" not found in repository. Make sure you have a fully checked out Half-Life: VR repository (https://github.com/maxmakesmods/Half-Life-VR), AND ensure you have installed all NuGet packages required by HLVRConfig.')
            sys.exit(2)

    for folder in folders_to_copy:
        if not os.path.exists(os.path.join(script_location, folder)):
            print('Error: Folder "'+folder+'" not found in repository. Make sure you have a fully checked out Half-Life: VR repository (https://github.com/maxmakesmods/Half-Life-VR).')
            sys.exit(2)
