...
void CFuncRotating::RampPitchVol(int fUp)
{
	...
	EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(pev->noiseRunning), fvol, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL, pitch);
}
...
void CFuncRotating::SpinUp(void)
{
		...
		fabs(vecAVel.z) >= fabs(pev->movedir.z * pev->speed))
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(pev->noiseRunning), m_flVolume, m_flAttenuation, SND_CHANGE_PITCH | SND_CHANGE_VOL, FANPITCHMAX);
...
void CFuncRotating::SpinDown(void)
{
		...
		((vecdir < 0) && (vecAVel.x >= 0 && vecAVel.y >= 0 && vecAVel.z >= 0)))
	{
		...
		EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(pev->noiseRunning /* Stop */), 0, 0, SND_STOP, m_pitch);
...
void CFuncRotating::RotatingUse(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value)
{
	...
	else if (!FBitSet(pev->spawnflags, SF_BRUSH_ACCDCC))  //this is a normal start/stop brush.
	{
		...
		{
			EMIT_SOUND_DYN(ENT(pev), CHAN_STATIC, STRING(pev->noiseRunning), m_flVolume, m_flAttenuation, 0, FANPITCHMAX);
