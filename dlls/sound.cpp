...
// Intercept PRECACHE_SOUND to precache female NPC sounds when precaching NPC sounds - Max Makes Mods, 2018-11-23
...
// Intercept EMIT_AMBIENT_SOUND to inject female NPC sounds when played as ambient sound (e.g. in the test lab) - Max Makes Mods, 2018-11-23
...
float TEXTURETYPE_PlaySound(TraceResult* ptr, Vector vecSrc, Vector vecEnd, int iBulletType, float volume)
{
	...
	UTIL_EmitAmbientSound(ENT(0), ptr->vecEndPos, rgsz[RANDOM_LONG(0, cnt - 1)], fvol * volume, fattn, 0, 96 + RANDOM_LONG(0, 0xf));
