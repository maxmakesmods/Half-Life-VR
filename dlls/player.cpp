...
{
	// Gun position and angles determined by attachment on weapon model - Max Makes Mods, 2019-03-30
	Vector pos;
	Vector angles;
	GET_ATTACHMENT(m_vrControllers[GetWeaponControllerID()].GetModel()->edict(), VR_MUZZLE_ATTACHMENT, pos, angles);
	ALERT(at_console, "CBasePlayer::GetAimPosition for %s: %f %f %f", STRING(m_vrControllers[GetWeaponControllerID()].GetModel()->pev->model), pos.x, pos.y, pos.z);
	return pos;
}
...
bool CBasePlayer::IsWeaponUnderWater()
{
	return UTIL_PointContents(GetWeaponPosition()) == CONTENTS_WATER
		|| UTIL_PointContents(GetGunPosition()) == CONTENTS_WATER;
}
