...
void AlertMessage(ALERT_TYPE atype, char* szFmt, ...)
{
	...
	vsprintf_s(string, szFmt, argptr);
	va_end(argptr);
...
=====================
*/
void CBaseEntity::Killed(entvars_t* pevAttacker, int bitsDamageType, int iGib)
{
...
void HUD_WeaponsPostThink(local_state_s* from, local_state_s* to, usercmd_t* cmd, double time, unsigned int random_seed)
{
	...
	if (g_runfuncs)
	{
		...
		{
			player.Killed(nullptr, DMG_GENERIC, 0);
...
// Client side implementations of entity functions for proper debugging (no more inline) - Max Makes Mods, 2020-03-04
